...
def consolidate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not singleview_df.empty:
        singleview_agg = singleview_df.groupby(
            ["method", "views", "scene"], as_index=False
        ).psnr.mean()
    ...
    wide = wide.rename(
        columns={
            s: f"{s}_psnr" for s in wide.columns if s not in ["method", "views", "dls"]
        }
    )
    ...
    wide[scene_cls + ["avg_psnr", "std_psnr"]] = wide[
        scene_cls + ["avg_psnr", "std_psnr"]
    ].round(3)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Input CSV file with runs data",
    )
