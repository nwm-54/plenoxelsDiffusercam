...
def remove_gaussians(
    ply_path: str,
    output_path: str,
    brightness_threshold_low: float = 0.1,
    brightness_threshold_high: float = 0.9,
    ...
    print(
        f"Found {black_count} black gaussians ({black_count / initial_count * 100:.2f}%)"
    )
    print(
        f"Found {white_count} white gaussians ({white_count / initial_count * 100:.2f}%)"
    )
    ...
    print(
        f"  Center of mass: [{center_of_mass[0]:.3f}, {center_of_mass[1]:.3f}, {center_of_mass[2]:.3f}]"
    )
    print(f"  Median distance: {median_distance:.3f}")
    print(f"  MAD: {mad:.3f}")
    print(
        f"  Distance threshold: {median_distance + distance_threshold * mad * 1.4826:.3f}"
    )
    print(
        f"Found {outlier_count} spatial outliers ({outlier_count / initial_count * 100:.2f}%)"
    )
    ...
    print(
        f"  Total to remove: {total_remove} ({total_remove / initial_count * 100:.2f}%)"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Remove black, white, and outlier gaussians from a PLY file"
    )
