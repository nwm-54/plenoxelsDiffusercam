...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, resolution, dls):
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        gt_image = rgb_gt #/torch.max(_gt_image) FOR real data # rgb_gt #for simulation only #_gt_image #/torch.max(_gt_image) 
       
        # Loss
        Ll1 = l1_loss(output_image, gt_image)  #use image_tensor, gt_tensor for non-multiplex
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(output_image, gt_image)) + tv_loss/len(tv_viewpoints)*opt.tv_weight + tv_train_loss/len(scene_train_cameras)*opt.tv_train_weight #+ negative_image*1e-4  + l2_loss(img1, img2)*(10*6)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                if config['name'] == 'test':
                    ...
                    {
                        "epoch": iteration,
                        "val_acc_adj": psnr_test,
                        "val_loss_adj": l1_test,
                    }
                )
                elif config['name'] == 'non adj test':
                    wandb.log(
                    {
                        "epoch": iteration,
                        "val_acc_non_adj": psnr_test,
                        "val_loss_non_adj": l1_test,
                    }
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[500,1_000, 2_000,2500, 3_000,4_000,7000, 7001,8000,10_000, 12_000, 14_000, 18_000,20_000, 27_000, 29_000,30_001,30_010,30_200,30_400,30_600,31_000,32_000,33_000,34_000,35_000,36_000,40_000, 50_000])
    # parser.add_argument("--save_iterations", nargs="+", type=int, default=[1,7_000, 30_000, 1_000,2_000,3_000,4_000,7000, 7001,7050,8000,10_000, 12_000, 14_000, 18_000,20_000, 27_000, 29_000,30_001,30_010,30_200,30_400,30_600,31_000,32_000,33_000,34_000,35_000,36_000,40_000, 50_000])
    ...
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[2000,2500])
    ...
    tv_train_weight = 0.0 #wandb.config.tv_train_weight
    args.resolution = 1
    args.dls = 18
    args.tv_weight = 0.002 #tv_weight
    args.tv_train_weight = tv_train_weight
    args.source_path = "/home/vitran/plenoxels/blender_data/lego_gen12"
    # args.source_path = "/home/vitran/plenoxels/blender_data/chair"
    args.iterations =3011
    args.model_path = f'./output/lego_800_dls18_tv_{tv_weight:.5f}'
    args.resolution = 1
    # args.model_path = f'./output/chair_800_tv_{tv_weight:.5f}'

    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.resolution, args.dls)
