...
def get_loss_rays(data_dict, rays, rays_mask, gt, resolution, radius, harmonic_degree, jitter, uniform, key, sh_dim, occupancy_penalty, interpolation, nv, white_bkgd=True):
    ...
    for k in range(rays[0].shape[1]):
        rgb, disp, acc, weights, voxel_ids = plenoxel.render_rays(data_dict, [rays[0][:,k,:], rays[1][:,k,:]], resolution, key, radius, harmonic_degree, jitter, uniform, interpolation, nv, white_bkgd)
        rgb_total += rgb*rays_mask[:,k]
        gt_total += gt[:,k,:]*rays_mask[:,k]
...
def render_pose_rays(data_dict, c2w, H, W, focal, resolution, radius, harmonic_degree, jitter, uniform, key, sh_dim, batch_size, interpolation, nv):
    ...
    for i in range(int(np.ceil(H*W/batch_size))):
        ...
        if jitter > 0:
            rgbi, dispi, acci, weightsi, voxel_idsi = jax.lax.stop_gradient(plenoxel.render_rays(data_dict, (rays_o[start:stop], rays_d[start:stop]), resolution, key[start:stop], radius, harmonic_degree, jitter, uniform, interpolation, nv, white_bkgd))
        else:
            rgbi, dispi, acci, weightsi, voxel_idsi = jax.lax.stop_gradient(plenoxel.render_rays(data_dict, (rays_o[start:stop], rays_d[start:stop]), resolution, None, radius, harmonic_degree, jitter, uniform, interpolation, nv, white_bkgd))
        rgbs.append(rgbi)
        disps.append(dispi)
...
if FLAGS.physical_batch_size is not None:
    ...
    mask = np.broadcast_to(mask, (10, mask.shape[1], mask.shape[2], mask.shape[3],3)) #mask.shape [10,800,800,MAX_PER_PIXEL]
    ...
    new_rays_rgb = np.zeros((800,800,10,MAX_PER_PIXEL, 2,3))#FLAGS.dim_lens instead of 800
    ...
    train_gt2 = multi_lowpass(train_gt[:,None,None], FLAGS.resolution)
    temp = np.transpose(train_gt2, (0,3,4,1,2,5))

    temp = np.broadcast_to(temp, (10, 800,800, MAX_PER_PIXEL,1,3))
    print(temp.shape)
...
def main():
    global rays_rgb, keys, render_keys, data_dict, FLAGS, radius, train_c2w, train_gt, test_c2w, test_gt, automatic_lr , rootkeys, cnt, log_file
    start_epoch = 0
    sh_dim = (FLAGS.harmonic_degree + 1)**2
            ...
            if FLAGS.physical_batch_size is None:
                ...
                for k in tqdm(range(len(rays_rgb) // FLAGS.logical_batch_size)):
                    ...
                    for j in range(FLAGS.logical_batch_size // FLAGS.physical_batch_size):
                        ...
                        mse, mse_grad = jax.value_and_grad(lambda grid: get_loss_rays((indices, grid), batch_rays, batch_mask, target_s, FLAGS.resolution, radius, FLAGS.harmonic_degree, FLAGS.jitter, FLAGS.uniform, subkeys, sh_dim, occupancy_penalty, FLAGS.interpolation, FLAGS.nv, white_bkgd))(data) 
